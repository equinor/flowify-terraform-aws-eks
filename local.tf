locals {
  ...
  node_security_group_id = coalesce(join("", aws_security_group.nodes.*.id), var.node_security_group_id,)
  default_iam_role_id       = element(concat(aws_iam_role.workers.*.id, [""]), 0)

  kubeconfig_name = var.kubeconfig_name == "" ? "eks_${var.cluster_name}" : var.kubeconfig_name
  ami_name_eks    = "${var.ami_name_eks}-${var.cluster_version}-${var.worker_ami_name_filter}"
  ...
  workers_group_defaults_defaults = {
    ...
    key_name                      = "dev-ops"                  # The key name that should be used for the instances in the autoscaling group
  ...
  node_group_launch_template_defaults_defaults = {
    name                                     = "count.index"                            # Name of the worker group. Literal count.index will never be used but if name is not set, the count.index interpolation will be used.
    tags                                     = []                                       # A list of map defining extra tags to be applied to the worker group ASG.
    ami_id                                   = data.aws_ami.eks_worker.id               # AMI ID for the eks workers. If none is provided, Terraform will search for the latest version of their EKS optimized worker AMI.
    root_block_device_id                     = data.aws_ami.eks_worker.root_device_name # Root device name for workers. If non is provided, will assume default AMI was used.
  ...
  node_group_launch_template_defaults = merge(local.node_group_launch_template_defaults_defaults, var.node_group_launch_template_defaults, )

################################################################
# Provides an AutoScaling Group resource using Launch Templates
################################################################
...
resource "aws_autoscaling_group" "workers_launch_template" {
  ...
  target_group_arns         = lookup(var.worker_groups_launch_template[count.index], "target_group_arns", local.workers_group_launch_template_defaults["target_group_arns"])
  ...
  suspended_processes       = lookup(var.worker_groups_launch_template[count.index], "suspended_processes", local.workers_group_launch_template_defaults["suspended_processes"])
  ...
  tags = concat(
    [
      ...
      {
        key                 = "k8s.io/cluster-autoscaler/${lookup(var.worker_groups_launch_template[count.index], "autoscaling_enabled", local.workers_group_launch_template_defaults["autoscaling_enabled"]) == 1 ? "enabled" : "disabled"}"
        value               = "true"
        propagate_at_launch = false
      },
    ],
    local.asg_tags,
    lookup(var.worker_groups_launch_template[count.index], "tags", local.workers_group_launch_template_defaults["tags"])
  )
}
...
resource "aws_launch_template" "workers_launch_template" {
  ...
  user_data     = base64decode(element(local.template_launch_template_userdata, count.index, ), )
  ...
  network_interfaces {
    ...
    security_groups             = flatten([local.worker_security_group_id, var.worker_additional_security_group_ids, lookup(var.worker_groups_launch_template[count.index], "additional_security_group_ids", local.workers_group_launch_template_defaults["additional_security_group_ids"])])
  }
  ...
  tag_specifications {
    ...
    tags = merge(
      {
        "Name" = "${aws_eks_cluster.this.name}-${lookup(var.worker_groups_launch_template[count.index], "name", count.index, )}-eks_asg"
      },
      var.common_tags,
    )
  }
...
resource "aws_iam_instance_profile" "workers_launch_template" {
  ...
  role        = lookup(var.worker_groups_launch_template[count.index], "iam_role_id", local.workers_group_launch_template_defaults["iam_role_id"], )
  count       = var.worker_group_launch_template_count
}
